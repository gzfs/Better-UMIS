...
export const StudentRegistrationWizard = () => {
	...
	const validateCurrentStep = (): boolean => {
		...
		switch (currentStep) {
			case 0: // General Information
				if (!formData.emisAvailable)
					errors.emisAvailable = "EMIS ID availability is required";
				if (
					formData.emisAvailable === "no" &&
					!formData.emisUnavailableReason
				) {
					errors.emisUnavailableReason =
						"Reason for EMIS ID unavailability is required";
				}
				if (!formData.salutation) errors.salutation = "Salutation is required";
				if (!formData.studentNameCertificate)
					errors.studentNameCertificate =
						"Student name (as on certificate) is required";
				if (!formData.studentNameAadhaar)
					errors.studentNameAadhaar =
						"Student name (as on Aadhaar) is required";
				...
				if (!formData.communityCertificateNumber)
					errors.communityCertificateNumber =
						"Community certificate number is required";
				if (!formData.aadhaarNumber)
					errors.aadhaarNumber = "Aadhaar number is required";
				if (!formData.firstGraduate)
					errors.firstGraduate = "First graduate status is required";
				if (!formData.specialQuota)
					errors.specialQuota = "Special quota status is required";
				if (!formData.differentlyAbled)
					errors.differentlyAbled = "Differently abled status is required";
				break;
			...
			case 1: // Contact Information
				...
				if (!formData.permanentAddress.state)
					errors.permanentState = "Permanent address state is required";
				if (!formData.permanentAddress.district)
					errors.permanentDistrict = "Permanent address district is required";
				if (!formData.permanentAddress.postalAddress)
					errors.permanentPostalAddress =
						"Permanent postal address is required";
				break;
			...
			case 2: // Family Information
				...
				if (!formData.familyAnnualIncome)
					errors.familyAnnualIncome = "Family annual income is required";
				if (!formData.parentMobileNumber)
					errors.parentMobileNumber = "Parent mobile number is required";
				break;
			...
			case 4: // Academic Information
				...
				if (!formData.branchSpecialization)
					errors.branchSpecialization = "Branch/Specialization is required";
	...
	const renderCurrentTab = () => {
		switch (currentStep) {
			...
			case 5:
				return (
					<CompletionTab formData={formData} currentToken={currentToken} />
				);
			default:
				return null;
		}
	...
	return (
		<Card className="w-full max-w-4xl mx-auto">
			...
			<CardContent>
				...
				{/* Current Tab Content */}
				<div className="min-h-96">{renderCurrentTab()}</div>
